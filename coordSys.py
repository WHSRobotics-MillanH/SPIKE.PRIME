...
#function
def gyro(desiredDegree, distance, deadband, ka=1, kv=10, rapidness=0, kp=0.7, ki=0.000125, kd=3, wheelLockLeft=False, wheelLockRight=False,dime=False,turnspeed=8,turndiff=3): #maybe switch kp to 1 to 3. turndiff maybe should be 3
    ...
    ryaw = resetyaw()
    error1=desiredDegree - quaternion_to_yaw(ogyaw)
    while True:
        quaternion = motion_sensor.quaternion()
        yawangle = yaw(quaternion,ogyaw)
        ticks=utime.ticks_ms()
        #Proportional
        ...
        if distance!=0:
            ...
            if round(distanceTraveled,0)==round(distance,0):
                stop(port.A,port.B)
                reset(port.A,port.B)
                break
        ...
        if yawangle>desiredDegree+deadband: #original deadband was 5
            ...
            if distance>0:
                ...
                reset(port.A,port.B)
                break
def coordSys(desiredX, desiredY, currentX=0,currentY=0):
